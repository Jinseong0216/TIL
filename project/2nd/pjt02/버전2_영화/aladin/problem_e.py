...
def get_users_books(title):
    ...
    if response['item'] == []: return None
        ...
        priceStandard = response['item'][0]['priceStandard']
        ...
        if min_Price == 0:
            ...
            f'도서 "{title}"의 중고 재고가 없으며, 새 제품은 {priceStandard}원에 판매 중입니다.'
        else:
            if i == 0:
                return f'도서 "{title}"의 가장 저렴한 중고는 알라딘이 보유 중이며, {min_Price}원에 판매 중입니다.'
            elif i == 1:
                return f'도서 "{title}"의 가장 저렴한 중고는 광활한 우주점이 보유 중이며, {min_Price}원에 판매 중입니다.'
            else:
                return f'도서 "{title}"의 가장 저렴한 중고는 개인 회원이 보유 중이며, {min_Price}원에 판매 중입니다.'
